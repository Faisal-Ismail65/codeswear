    ...
    <div>
      <header className="text-gray-600 body-font ">
        <div className="container mx-auto flex flex-wrap p-5 flex-col md:flex-row items-center sticky">
          ...
          <div className='flex  cursor-pointer mr-40' >
            <span onMouseOver={() => { setDropdown(true) }} onMouseLeave={() => { setDropdown(false) }}>
              {dropdown && <div onMouseOver={() => { setDropdown(true) }} onMouseLeave={() => { setDropdown(false) }} className="absolute  right-48 top-14 bg-rose-300 p-2 rounded-lg w-36">
                <ul>
                  <Link href={'/myaccount'}><a><li className='py-1 text-sm hover:text-rose-700'>My Account</li></a></Link>
                  <Link href={'/orders'}><a><li className='py-1 text-sm hover:text-rose-700'>Orders</li></a></Link>
                  <li onClick={props.logout} className='py-1 text-sm hover:text-rose-700'>Logout
                  </li>
                </ul>
                <ToastContainer />
              </div>}

              {props.user.value && <MdAccountCircle className='text-4xl' />}
            </span>
            {!props.user.value && <Link href={'/login'} ><a>
              <button className='items-center bg-rose-500 font-bold  border-0 py-1 px-3 focus:outline-none hover:text-yellow-900    hover:bg-gray-200 rounded text-xl mt-4 md:mt-0' >Login</button>
            </a></Link>}
      ...
      <div ref={ref} className="w-72 overflow-y-scroll  z-10 sideCart absolute h-full  bg-rose-300 top-5 right-0 px-8 py-10 transform transition-transform translate-x-full ">
          ...
          {Object.keys(props.cart).map((k) => {
            return <li key={k}>
              <div className="item flex">
                <div className="w-2/3 font-semibold">{props.cart[k].name}({props.cart[k].size}/{props.cart[k].varient})</div>
