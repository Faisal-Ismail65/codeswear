          ...
          <div className="lg:w-1/2 w-full lg:pl-10 lg:py-6 mt-6 lg:mt-0">
            ...
            <div className="flex mt-6 items-center pb-5 border-b-2 border-gray-100 mb-5">
              <div className="flex">
                ...
                {Object.keys(variants).includes('red') && Object.keys(variants['red']).includes(size) && <button onClick={() => { refreshVariant(size, 'red') }} className={` border-2  ml-1 bg-rose-500 rounded-full w-6 h-6 focus:outline-none ${color === 'red' ? 'border-black' : 'border-gray-300'} `}></button>}
              ...
              <div className="flex ml-6 items-center">
                ...
                <div className="relative">
                  <select value={size} onChange={(e) => { refreshVariant(e.target.value, color) }} className="rounded border appearance-none border-gray-300 py-2 focus:outline-none focus:ring-2 focus:ring-rose-200 focus:border-rose-500 text-base pl-3 pr-10">
            ...
            <div className="flex">
              ...
              <button onClick={()=>{buyNow(slug, 1, product.price, product.title, product.size, product.color)}} className="ml-6 text-white bg-rose-500 border-0 py-1 px-2 focus:outline-none hover:bg-rose-600 rounded text-md font-semibold">Buy Now</button>
              <button onClick={() => { addToCart(slug, 1, product.price, product.title, product.size, product.color) }} className="flex ml-4 text-white bg-rose-500 border-0 py-2 px-2 focus:outline-none hover:bg-rose-600 rounded text-md font-semibold">Add to Cart </button>
            ...
            <div className="pin mt-6 flex space-x-2 text-sm">
              <input onChange={onChangePin} type="text" className='px-2 border-2 border-rose-500 rounded-xl' />
              <button onClick={checkServiceability} className=' text-white bg-rose-500 border-0 py-2 px-6 focus:outline-none hover:bg-rose-600 rounded text-lg'>Enter Pincode </button> 
            </div>
            {!service && service != null && <div className='mt-2 text-md text-rose-600'>
              Sorry! We do not deliver to this Pincode yet</div>}
            {service && service != null && <div className=' mt-2 text-md text-rose-600'>This pincode is serviceable</div>}
          </div>
        </div>
      </div>
    </section>
